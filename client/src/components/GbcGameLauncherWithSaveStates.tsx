...
const GbcGameLauncherWithSaveStates: React.FC = () => {
    ...
    useEffect(() => {
        try {
            const savedStates = localStorage.getItem('gbcGameSaveStates');
            if (savedStates) {
    ...
    const launchGame = async () => {
        try {
            const response = await fetch('/roms/PKMNC.gbc');
            const romData = await response.blob();
            
            const nostalgist = await Nostalgist.gbc({
                fileContent: romData,
                fileName: 'PKMNC.gbc'
            });
    ...
    const handleSaveState = async () => {
        ...
        try {
            ...
            localStorage.setItem('gbcGameSaveStates', JSON.stringify(statesForStorage));
    ...
    return (
        <div className="game-launcher-page">
            <div className="controls-container">
                <h2 className="text-xl font-bold mb-4">Launch Pokemon Crystal Version</h2>
